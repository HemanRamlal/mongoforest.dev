...
export default function Submissions({ problemSlug }) {
  ...
  return <div className="submissions-page-wrapper">
    <div className="submissions-page">
      ...
      <div className="submissions-list">
        ...
        {submissions.length == 0 && <div class="empty-submissions-indicator">
          No submissions to show
        </div>}
        {submissions.map(submission => {
          const verdictInfo = getVerdictDisplay(submission.verdict);
          return (
            <div key={submission.id} className={`submission-item ${view?.id == submission.id ? "submission-item-active" : ""}`} onClick={() => {
              setView(submission)
              smoothScrollToTop();
            }}>
              <div className="submission-id"><span className="num">#{submission.id}</span></div>
      ...
      <div className="submissions-pagination-menu">
        <Pagination page={page} onChange={(e, newPage)=>setPage(newPage)} count={Math.ceil(submissionsQuery.data.length/10)}/>
      </div>

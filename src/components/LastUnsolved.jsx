...
export default function LastUnsolved() {
  ...
  const {data, error, isPending, isError} = useQuery(lastUnsolvedQueryOptions({usernamem : user.username}));
  ...
  return (
    <div className="lastUnsolved">
      ...
      {list.length==0 && <div className="lastUnsolved-nolist">
        No Attempted Unsolved Problems
      </div>}
      {list.length!=0 && <div className="lastUnsolved-list">

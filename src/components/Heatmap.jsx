...
function putSvg(parentSelector, dateActivityMap, year) {
  ...
  const dataset = days.map(day => {
    return [
      ...
      dateActivityMap.find(entry => {
        ...
        return dateFns.format(submissionTime, "MM-dd-yyyy") == dateFns.format(targetDay, "MM-dd-yyyy");
      }) ?? { count: 0 },
      day]
  });
  ...
  const svgRoot = d3.select(parentSelector).append('svg').attr('width', width + margin.left + margin.right + totalGaps).attr('height', height * 2 + margin.top * 2 + margin.bottom);
  const topSvg = svgRoot.append('g').attr('transform', `translate(${margin.left}, ${margin.top})`);
  const bottomSvg = svgRoot.append('g').attr('transform', `translate(${margin.left}, ${margin.top * 2 + height})`);
  ...
  const xBottom = d3.scaleBand()
    .range([0, width])
    .domain(d3.range(27, 54))
    .padding(pad);

  const yBottom = d3.scaleBand()
    .range([0, height])
    .domain(weekDays)
    .padding(pad);
  ...
  topSvg.selectAll()
    .data(dataset.slice(0, daysUntilJuly), function (data) { return data[0] + ":" + data[1] })
    .join('rect')
    .attr('x', (data) => {
  ...
  bottomSvg.selectAll()
    .data(dataset.slice(daysUntilJuly), function (data) { return data[0] + ":" + data[1] })
    .join('rect')
    .attr('x', (data) => {
      console.log(data[0]);
  ...
  topSvg.selectAll()
    .data(months.slice(0, 6), function (d) { return 'label-' + d; })
    .join('text')
    .text(d => d)
  ...
  bottomSvg.selectAll()
    .data(months.slice(6), function (d) { return 'label-' + d; })
    .join('text')
    .text(d => d)
    .attr('text-anchor', 'center')
    .attr('y', height + 15)
    ...
    <main>
      ...
      <select id="year-selector" defaultValue={heatmapYear} onChange={(e)=>{
        ...
        <div className="heatmap-control">
          ...
          <select id="mode-selector" defaultValue={mode} onChange={(e) => {

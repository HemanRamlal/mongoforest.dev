...
import { faCheck, faPencil, faGavel, faCrown, faUserPlus, faUserMinus, faArrowRightFromBracket, faRightToBracket, faHourglass } from '@fortawesome/free-solid-svg-icons';
...
export default function Leaderboard() {
  ...
  const thisLeaderboardId = ['leaderboard', {
  ...
  async function demoteToMemberMutationFn({ toDemote }) {
    const res = await api.post(
      `/community/${communityInfo.id}/edit/removeAdmin/${toDemote}`,
      {},
  ...
  async function promoteToAdminMutationFn({ toPromote }) {
    const res = await api.post(
      `/community/${communityInfo.id}/edit/addAdmin/${toPromote}`,
      {},
  ...
  async function communityInfoMutator(variables, { client }) {
    ...
    if (variables.name) {
      ...
      await api.post(
        `/community/${communityInfo.id}/edit/name`,
        { name: variables.name },
  ...
  async function onCommunityInfoError(err, variables, rollback, { client }) {
    ...
    if (rollback.avatar) {
      ...
      if (!communityInfo.admins.includes(rollback.addAdmin)) communityInfo.admins.push(rollback.addAdmin);
  ...
  async function addMemberMutationFn({ newMemberName }) {
    ...
    const res = await api.post(
      `/community/${communityInfo.id}/edit/addMember/${user.id}`,
      {},
  ...
  async function kickMemberMutationFn({ kickeeId }) {
    const res = await api.post(
      `/community/${communityInfo.id}/edit/removeMember/${kickeeId}`,
      {},
  ...
  async function onLeaderboardError(err, variables, onMutateResult, { client }) {
    ...
    newLeaderboard = newLeaderboard.filter(entry => !toDel.some(toDel_entry => toDel_entry.user_id == entry.user_id));
  ...
  useEffect(() => {
    ...
    if (communityInfoQuery.data.admins && communityInfoQuery.data.admins.includes(getUser ? getUser.id : 'dummy')) {
  ...
  function handleAddUserKeyDown(e) {
    if (e.key === "Enter") addMemberMutation.mutate({
      ...
      <div className="leaderboard-page">
        <div className="leaderboard-banner">
          {adminMode &&
            ...
            />}
          {!adminMode &&
            <img src={communityInfoQuery.data.avatar} />
          }
          {!editingName &&
            <div className="leaderboard-community-name"
              onClick={
                () => {
              ...
              {adminMode && <sup className="leaderboard-community-name-edit-pencil"><FontAwesomeIcon icon={faPencil} /></sup>}
            </div>}
          {editingName && <div className="leaderboard-community-name-edit-wrap">
            <input autoFocus className="leaderboard-community-name" value={draftName} onChange={(e) => {
              setDraftName(e.target.value);
            }} onKeyDown={handleCommunityNameFieldEnter} />
            <button className="leaderboard-community-name-edit" onClick={() => { nameMutation.mutate({ name: draftName }) }}>
        ...
        <div className="leaderboard-user-controls">
          {isAdmin &&
            <div className="community-add-member">
              <input type="text" placeholder="New member's username" value={newMemberName} disabled={addMemberMutation.isPending} onChange={(e) => { setNewMemberName(e.target.value) }} onKeyDown={handleAddUserKeyDown} />
              <div className={`add-member-submit ${addMemberMutation.isPending && 'add-member-submit-disabled'}`} onClick={addMemberMutation.isPending ? null : () => addMemberMutation.mutate({ newMemberName })}>
                <FontAwesomeIcon icon={faUserPlus}></FontAwesomeIcon> {addMemberMutation.isPending ? "Adding User" : "Add User"}
              </div>
            </div>}
        ...
        {(isAdmin || isMember) &&
          <div className="leaderboard-user-info">
            {communityInfoQuery.data.name != "global" &&
              <span>You are <b> {isAdmin ? "an admin" : "a standard"} </b> {isMember && " member "} of this community </span>}
            {isMember == true && communityInfoQuery.data.name != 'global' &&
              <ControlButton disabled={exitCommunityMutation.isPending} className={`community-leave`} onClick={() => exitCommunityMutation.mutate({ exitCommunity: true })}>
                <FontAwesomeIcon icon={faArrowRightFromBracket} />{exitCommunityMutation.isPending ? "Exiting Community" : "Exit Community"}
              </ControlButton>}
            {isMember == false &&
              <ControlButton className={`community-join`} disabled={joinCommunityMutation.isPending} onClick={() => joinCommunityMutation.mutate({ joinCommunity: true })}>
        ...
        <div className="leaderboard-main">
          ...
          {!leaderboardQuery.isPending && leaderboardQuery.isFetching && <><div class="leaderboard-progress-container"><LinearProgress sx={{ width: "100%" }} /></div><span>{/*Because LinearProgress alone messes up odd even colors of .leaderboard-main-row's*/}</span></>}
          {leaderboardQuery.isPending && <LeaderboardFallback />}
          {leaderboardQuery.data && leaderboardQuery.data.map(user => {
            return <div className={`leaderboard-main-row`} id={getUser && user.user_id == getUser.id ? 'leaderboard-main-row-active-user' : ''}>
              ...
              <div className="leaderboard-item-username">{
                ...
                {adminMode &&
                  <div className="leaderboard-item-username-manage">
                    <ControlButton
                      disabled={kickMemberMutation.isPending && kickMemberMutation.variables.kickeeId == user.user_id}
                      color={user.is_admin ? "orange" : "darkblue"}
                      onClick={user.is_admin ? () => demoteToMemberMutation.mutate({ toDemote: user.user_id }) : () => promoteToAdminMutation.mutate({
                        toPromote: user.user_id
                      })}

                    >
                      <FontAwesomeIcon icon={user.is_admin ? faGavel : faCrown} />
                      <span class="leaderboard-item-username-manage-text">{user.is_admin ? "Demote" : "Promote"}</span>
                    </ControlButton>
                    <ControlButton
                      disabled={kickMemberMutation.isPending && kickMemberMutation.variables.kickeeId == user.user_id}
                      color="red"
                    ...
                    >
                      ...
                      <span class={`leaderboard-item-username-manage-text`}>{kickMemberMutation.isPending && kickMemberMutation.variables.kickeeId == user.user_id ? "Kicking" : "Kick"}</span>
                    </ControlButton>
